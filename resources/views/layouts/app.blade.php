...
<body class="bg-gray-50 font-sans text-gray-800 flex flex-col min-h-screen">
    ...
    <nav class="bg-white/95 backdrop-blur-lg shadow-sm sticky top-0 z-50 border-b border-gray-100 transition-all duration-300">
        <div class="max-w-[1400px] mx-auto px-4 py-3 flex justify-between items-center gap-4 lg:gap-6">
            ...
            <div class="flex items-center gap-4 lg:gap-6 shrink-0" x-data="{ showMenu: false, activeCategory: {{ $navRootCategories->first()->id ?? 1 }} }">
                ...
                <button @click="showMenu = !showMenu" class="w-11 h-11 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-all duration-300 shadow-sm outline-none group">
                ...
                <a href="{{ url('/') }}" class="flex flex-col justify-center leading-none group" title="Về trang chủ">
                    <div class="flex items-center gap-2.5">
                        <div class="w-10 h-10 bg-gradient-to-br from-green-400 to-emerald-600 rounded-xl flex items-center justify-center text-white text-xl shadow-md shadow-emerald-200/50 transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-6">
                            <i class="fa-solid fa-hand-holding-hand"></i>
                        </div>
                        <span class="text-3xl font-black uppercase tracking-tighter transition-colors duration-300">
                    ...
                    <span class="text-[10px] text-gray-400 tracking-widest uppercase font-bold mt-1.5 transition-colors group-hover:text-emerald-500">Mua bán đồ cũ</span>
                </a>
                ...
                <div x-show="showMenu" @click.away="showMenu = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2"
                    ...
                    <div class="w-[35%] bg-gray-50/50 overflow-y-auto border-r border-gray-100 no-scrollbar">
                        <div class="p-6 font-black text-lg border-b border-gray-100 text-emerald-700 tracking-tight sticky top-0 bg-gray-50/90 backdrop-blur z-10 uppercase">Khám phá</div>
                        <ul class="py-3">
                            ...
                            <li>
                                <a href="{{ url('/?category_id=' . $parentCat->id) }}"
                                    ...
                                    :class="activeCategory === {{ $parentCat->id }} ? 'bg-white text-emerald-700 shadow-sm border-r-4 border-emerald-500' : 'text-gray-600 hover:bg-gray-100 border-r-4 border-transparent'"
                                    class="px-6 py-3 cursor-pointer flex items-center gap-4 transition-all block">
                                    <div class="w-9 h-9 rounded-full flex items-center justify-center transition-colors shadow-sm" :class="activeCategory === {{ $parentCat->id }} ? 'bg-emerald-100 text-emerald-600' : 'bg-white text-gray-400 border border-gray-100'">
                                        <i class="fa-solid {{ $icon }} text-sm"></i>
                                    </div>
                                    <span class="font-bold text-sm" :class="activeCategory === {{ $parentCat->id }} ? 'text-emerald-700' : 'text-gray-700'">{{ $parentCat->name }}</span>
                                </a>
                            </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="w-[65%] p-8 overflow-y-auto bg-white no-scrollbar relative">
                        ...
                        <div x-show="activeCategory === {{ $parentCat->id }}" class="h-full" style="display: none;">
                            ...
                            <div class="flex items-center justify-between mb-6 border-b border-gray-100 pb-3">
                                ...
                                <a href="{{ url('/?category_id=' . $parentCat->id) }}" class="text-xs font-bold text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg transition-colors">Xem tất cả <i class="fa-solid fa-arrow-right text-[10px] ml-1"></i></a>
                            </div>
                            <div class="grid grid-cols-2 gap-3">
                                @foreach($parentCat->children as $childCat)
                                <a href="{{ url('/?category_id=' . $childCat->id) }}" class="group flex items-center p-3 bg-gray-50 rounded-xl hover:bg-emerald-50 hover:shadow-sm border border-transparent hover:border-emerald-100 transition-all">
                                    <div class="w-2 h-2 rounded-full bg-gray-300 group-hover:bg-emerald-500 mr-3 transition-colors"></div>
                                    <span class="text-sm font-semibold text-gray-700 group-hover:text-emerald-700">{{ $childCat->name }}</span>
                                </a>
                                @endforeach
                            </div>
                            @else
                            <div class="flex flex-col items-center justify-center h-full text-center mt-[-20px]">
                                <div class="w-32 h-32 bg-gradient-to-tr from-emerald-100 to-green-50 rounded-full flex items-center justify-center text-emerald-500 text-5xl mb-6 shadow-inner border-4 border-white">
                                ...
                                <a href="{{ url('/?category_id=' . $parentCat->id) }}" class="bg-emerald-600 text-white px-8 py-3 rounded-full font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-200 transform hover:-translate-y-1 flex items-center gap-2">
            ...
            {{-- 2. Thanh tìm kiếm --}}
            <form action="{{ url('/') }}" method="GET" class="flex-1 max-w-2xl hidden md:flex items-center bg-gray-100/80 rounded-full border border-transparent focus-within:border-emerald-400 focus-within:bg-white focus-within:ring-4 focus-within:ring-emerald-50 transition-all duration-300 h-11 overflow-hidden">
            ...
            <div class="flex items-center gap-1 lg:gap-3 shrink-0">
                ...
                <a href="{{ route('cart.index') }}"
                    ...
                    class="w-12 lg:w-14 h-14 flex flex-col justify-center items-center gap-1 text-gray-500 hover:text-emerald-600 rounded-xl hover:bg-emerald-50 transition-all group relative">
                    <div class="relative">
                        ...
                        <span x-show="cartCount > 0"
                            ...
                            x-transition.scale
                            class="absolute -top-2 -right-2.5 bg-red-500 text-white text-[10px] font-black rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center border-2 border-white shadow-sm"
                            style="display: none;">
                        </span>
                    </div>
                ...
                @else
                <button type="button" onclick="window.location.href='{{ route('login') }}'" class="w-12 lg:w-14 h-14 flex flex-col justify-center items-center gap-1 text-gray-500 hover:text-emerald-600 rounded-xl hover:bg-emerald-50 transition-all group relative bg-transparent border-none cursor-pointer outline-none">
                    <div class="relative">
                        <i class="fa-solid fa-comment-dots text-xl transition-transform duration-300 group-hover:scale-110"></i>
                    </div>
                    <span class="text-[10px] font-bold hidden sm:block">Tin nhắn</span>
                ...
                @else
                <button type="button" onclick="window.location.href='{{ route('login') }}'" class="hidden xl:flex w-12 lg:w-14 h-14 flex-col justify-center items-center gap-1 text-gray-500 hover:text-orange-500 rounded-xl hover:bg-orange-50 transition-all group relative border-none outline-none cursor-pointer">
                    <div class="relative">
                        <i class="fa-solid fa-bell text-xl transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-12"></i>
                    </div>
                    <span class="text-[10px] font-bold">Thông báo</span>
                ...
                {{-- Đăng tin --}}
                <a href="{{ route('products.create') }}" class="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-4 lg:px-5 py-2.5 rounded-full font-bold hover:from-green-600 hover:to-emerald-700 transition-all shadow-lg shadow-emerald-200/50 flex items-center gap-2 transform hover:-translate-y-0.5">
                    ...
                    <div class="relative group py-2" x-data="{ open: false }">
                        <div class="flex items-center gap-3 cursor-pointer bg-white border border-gray-100 p-1.5 lg:pr-4 rounded-full shadow-sm hover:shadow transition-all" @click="open = !open" @click.away="open = false">
                            <div class="relative">
                                ...
                                @else
                                <div class="w-9 h-9 bg-emerald-100 text-emerald-700 rounded-full flex items-center justify-center font-bold text-lg border-2 border-white uppercase">
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 translate-y-2" x-transition:enter-end="opacity-100 scale-100 translate-y-0"
                            ...
                            <div class="bg-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden py-2">
                                ...
                                <a href="{{ url('/dashboard') }}" class="flex items-center gap-3 px-5 py-2.5 text-sm font-medium text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition mx-2 rounded-xl group">
                                    <i class="fa-solid fa-chart-line w-5 text-center transition-transform group-hover:scale-110"></i> Trang quản lý
                                </a>
                                <a href="{{ route('shop.show', Auth::id()) }}" class="flex items-center gap-3 px-5 py-2.5 text-sm font-medium text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition mx-2 rounded-xl group">
                                    <i class="fa-solid fa-store w-5 text-center transition-transform group-hover:scale-110"></i> Xem trang cá nhân
                                </a>
                                <a href="{{ url('/profile') }}" class="flex items-center gap-3 px-5 py-2.5 text-sm font-medium text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition mx-2 rounded-xl group">
    ...
    <footer class="bg-white border-t border-gray-100 mt-16 pt-16 pb-8">
        <div class="max-w-[1400px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
            <div>
                {{-- LOGO --}}
                <a href="{{ url('/') }}" class="flex flex-col justify-center leading-none group mb-5 inline-flex" title="Về trang chủ">
                    <div class="flex items-center gap-2.5">
                        <div class="w-10 h-10 bg-gradient-to-br from-green-400 to-emerald-600 rounded-xl flex items-center justify-center text-white text-xl shadow-md shadow-emerald-200/50 transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-6">
                            <i class="fa-solid fa-hand-holding-hand"></i>
                        </div>
            ...
            <div>
                ...
                <ul class="space-y-4 text-sm text-gray-500 font-medium">
                    ...
                    <li class="flex items-start gap-3">
                        <div class="w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0"><i class="fa-solid fa-envelope"></i></div>
                        <div>
                            <p class="text-xs text-gray-400">Email chăm sóc</p>
                            <p class="text-gray-900 font-bold">support@2hand.vn</p>
                        </div>
                    </li>
                </ul>
            </div>
